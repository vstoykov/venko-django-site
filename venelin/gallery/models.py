...
class Picture(models.Model):
    ...
    gallery = models.ForeignKey(
        Gallery, verbose_name=_('gallery'), related_name='pictures', on_delete=models.CASCADE
    )
    title = models.CharField(
        _('title'), max_length=255, blank=True, default='', help_text=_("Title of the picture")
    )
    image = ProcessedImageField(
        verbose_name=_('image'),
        max_length=255,
        upload_to=upload_picture_to,
        processors=[ResizeToFit(MAX_WIDTH, MAX_HEIGHT)],
        format='JPEG',
        options={'quality': 95},
    )
    thumb = ImageSpecField(
        source='image',
        processors=[ResizeToFit(MAX_THUMB_WIDTH, MAX_THUMB_HEIGHT)],
        format='JPEG',
        options={'quality': 60},
    )

    is_album_logo = models.BooleanField(
        _('is album logo'),
        default=False,
        help_text=_("If this is checked this picture will be the album logo"),
    )
    ...
    def preview(self):
        ...
        try:
            ...
            warnings.warn(err)
            return format_html(
                '<span class="help help-tooltip" title="{error}">N/A</span>', error=err
            )
        return format_html(
            '<a href="{image_url}" title="{title}"><img src="{thumb_url}" width="{width}" height="{height}" alt="{title}" /></a>',
